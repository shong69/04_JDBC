...
public class CommentView {
	...
	private void insertComment(int boardNo) {
		System.out.println("[댓글 입력]<!wq 입력 시 종료됨>");
		
		//String : 불변성을 지님 -> 이어쓰는게 아니라 계속 내용이 새로 생성됨
		//StringBuffer : 가변성 
		
		StringBuffer sb = new StringBuffer();
		
		//특정 단어가 입력될 때까지 무한히 입력되도록
		while(true) {
			String str = sc.nextLine();
			
			if(str.equals("!wq")) break;
			
			//append : StringBuufer의 제일 뒤에 추가하기
	...
	private void updateComment(int boardNo) {
		...
		try {
			...
			//입력한 댓글 번호가 해당 게시글에 대해 로그인한 회원이 작성한 댓글이 맞는지 확인하기
			...
			System.out.println("[댓글 수정 내용 입력] <!wq 입력 시 종료됨>");
